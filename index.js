...
function roundObjectValues(obj) {
  ...
  const round2 = Object.fromEntries(round);
  return round2;
...
function getFractionalPart(num) {
  ...
  const fullNumber = Math.trunc(num);
  const fractional = num - fullNumber;
  const result = Math.fround(fractional);
  return result;
...
function compareAndRound(num1, num2) {
  ...
  const round = Math.round(maxNumber);
  return round;
